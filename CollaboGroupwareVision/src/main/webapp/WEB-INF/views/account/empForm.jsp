			...
			<div class="card-body">
				...
				<form class="row g-3" action="/account/register" method="post"
					...
					<div class="col-md-6">
						<label for="positionCode" class="form-label">직위<span class="text-danger">*</span></label> <select
							...
							<c:forEach var="entry" items="${positionMap}">
							    <option value="${entry.key}" <c:if test="${emp.positionCode eq entry.key}">selected</c:if>>${entry.value}</option>
							</c:forEach>
						</select>
					</div>
						...
						<div class="col-md-6">
							<label for="dutyCode" class="form-label">직책</label> <select
								...
								<c:forEach var="entry" items="${dutyMap}">
								    <option value="${entry.key}" <c:if test="${emp.dutyCode eq entry.key}">selected</c:if>>${entry.value}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="col-12 gap-1 pt-3 d-flex justify-content-center">
						...
						<button type="button" id="clearBtn" class="btn btn-dark btn-lg">초기화</button>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	clearBtn.addEventListener("click", setInitialValue);
