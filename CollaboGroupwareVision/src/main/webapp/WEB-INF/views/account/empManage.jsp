...
function excelDown() {
	gridApi.exportDataAsExcel({
		columnKeys: ['empNo', 'empName', 'deptName', 'position', 'empEmail', 'empTel', 'hffc']
	});
}

// 사원 목록 불러오기
async function getEmpListData() {
	try {
		const response = await fetch('/account/getEmpList');
		if (!response.ok) {
	 ...
	 alertError('데이터 로드 실패');
	}
}

// 사원 비활성화
async function empDisable() {
	let selectedRows = gridApi.getSelectedRows();
    let empNoList = selectedRows.map(row => row.empNo);
    
    if(selectedRows.length == 0) {
    	alertWarning('사원을 선택해주세요!');
    	return;
    }
    ...
    if (result.isConfirmed) {
        try {
          const response = await fetch('/account/empDisable', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json;charset=UTF-8',
              [header]: token, // CSRF 토큰 헤더
            },
            body: JSON.stringify(empNoList),
          });
          ...
          alertSuccess('비활성화가 완료되었습니다.');
          getEmpListData();
          
        } catch (error) {
          alertError('비활성화에 실패했습니다.');
        }
    }
}
