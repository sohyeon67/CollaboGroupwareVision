			...
			<form id="projectForm" action="/adminProject/create" method="post" class="flex-fill">
				<div class="card-body mt-3 overflow-scroll scrollable-content" style="min-height: 700px;max-height: 700px;">
					...
					<label for="projectStatus" class="form-label">상태</label> <select
						...
						<c:forEach var="entry" items="${projectStatusMap}">
						    <option value="${entry.key}" <c:if test="${project.projectStatus eq entry.key}">selected</c:if>>
						        ${entry.value}
						    </option>
						</c:forEach>
					</select><br/>
...
$(function() {
	...
	$createBtn.on("click", function() {
		...
		if(name == "" || name.length == 0) {
			alertWarning('프로젝트명을 입력해주세요.');
			return;
		}
		
		if(startDay == "" || startDay.length == 0) {
			alertWarning('프로젝트 시작일을 입력해주세요.');
			return;
		}
		
		if(endDay == "" || endDay.length == 0) {
			alertWarning('프로젝트  종료일을 입력해주세요.');
			return;
		}
		...
		if(status == "" || status.length == 0) {
			alertWarning('프로젝트 상태를 입력해주세요.');
			return;
		}

		if(memCount === 0) {
			alertWarning('프로젝트 멤버를 추가해주세요.');
			return;
		}
...
function removeProjectMember(button) {
    ...
    reindexProjectMembers();
    showEmptyText();
}

//인덱스 재정렬 함수
function reindexProjectMembers() {
	document.querySelectorAll('.memListTr').forEach((tr, index) => {
		tr.querySelectorAll('input, select').forEach(el => {
			const name = el.getAttribute('name');
			if(name) {
				const newName = name.replace(/projectMemList\[\d+\]/, `projectMemList[\${index}]`);
...
async function projectEditFormModalOpen(projectId) {
	...
	try {
		const response = await fetch(`/project/getProjectDetails/\${projectId}`);
		if(!response.ok) throw new Error("서버 오류");
		
		const project = await response.json();
		$("#projectName").val(project.projectName);
		$("#projectDescription").val(project.projectDescription);
		$("#projectStartDay").val(getInputTypeDateDatas(project.projectStartDay));
		$("#projectEndDay").val(getInputTypeDateDatas(project.projectEndDay));
		$("#projectStatus").val(project.projectStatus);
		
		project.projectMemList.forEach(function(mem) {
			let memIndex = $memList.find('input[name^="projectMemList"]').length;
			console.log("멤버 인덱스 ", memIndex);
			let str = `
				<tr class="memListTr">
		...
		alertError('프로젝트 상세 정보를 불러오지 못했습니다.');
	}
