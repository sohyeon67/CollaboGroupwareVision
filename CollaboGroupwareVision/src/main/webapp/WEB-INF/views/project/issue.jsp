							...
							<div class="col-md-3">
								<label for="issueType" class="form-label">유형</label> <select
									...
									<c:forEach var="entry" items="${issueTypeMap}">
									    <option value="${entry.key}" <c:if test="${search.issueType eq entry.key}">selected</c:if>>
									        ${entry.value}
									    </option>
									</c:forEach>
								</select>
							...
							<div class="col-md-3">
								<label for="issueStatus" class="form-label">상태</label> <select
									...
									<c:forEach var="entry" items="${issueStatusMap}">
									    <option value="${entry.key}" <c:if test="${search.issueStatus eq entry.key}">selected</c:if>>
									        ${entry.value}
									    </option>
									</c:forEach>
								</select>
							...
							<div class="col-md-3">
								<label for="issuePriority" class="form-label">우선순위</label> <select
									...
									<c:forEach var="entry" items="${issuePriorityMap}">
									    <option value="${entry.key}" <c:if test="${search.issuePriority eq entry.key}">selected</c:if>>
									        ${entry.value}
									    </option>
									</c:forEach>
								</select>
							...
							<c:if test="${not empty issueList }">
								<c:forEach items="${issueList }" var="issue">
									<tr>
										...
										<td><a href="/project/${project.projectId }/issue/${issue.issueNo }"><c:out
