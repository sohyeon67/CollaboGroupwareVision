...
public class ProjectServiceImpl implements ProjectService {
	...
	public ServiceResult update(Project project) {
		...
		// null인 멤버 제거
		List<ProjectMember> filteredMembers = project.getProjectMemList()
			.stream()
			.filter(mem -> StringUtils.isNotBlank(mem.getEmpNo()))
			.collect(Collectors.toList());
		project.setProjectMemList(filteredMembers);
	...
	@Override
	public ServiceResult updateIssueDate(Issue issue) {
		int status = projectMapper.updateIssueDate(issue);
		if(status > 0) {
